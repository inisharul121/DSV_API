...
const formatPhoneNumber = (num) => {
    ...
    // If it already matches the DSV pattern (digits + space + digits), return as is
    if (/^[+]?[0-9]{1,4} [0-9]{1}/.test(num)) {
        return num;
    }
    ...
    // If starts with +, try to insert space after first 2 or 3 digits
    if (clean.startsWith('+')) {
        // +4178... -> +41 78... (Swiss example)
        if (clean.length > 5) {
            // Check common 2-digit country codes first
            const common2 = ['+1', '+7'];
            const common3 = ['+41', '+44', '+49', '+45', '+33', '+39', '+31', '+32', '+34', '+61', '+81', '+86', '+91', '+971'];

            for (const c of common2) {
                if (clean.startsWith(c)) return c + ' ' + clean.substring(c.length);
            }
            for (const c of common3) {
                if (clean.startsWith(c)) return c + ' ' + clean.substring(c.length);
            }

            // Generic: space after + and first 2 digits
            return clean.substring(0, 3) + ' ' + clean.substring(3);
        }
    } else if (clean.length > 5) {
        // No +, maybe it's 0041... or just local
        if (clean.startsWith('00')) {
            return '+' + clean.substring(2, 4) + ' ' + clean.substring(4);
        }
        // Local fallback: just insert a space after 3 digits so it passes regex
        return clean.substring(0, 3) + ' ' + clean.substring(3);
    }
...
exports.buildBookingPayload = (data) => {
    ...
    const payload = {
        ...
        sender: {
            ...
            registrationNumbers: data.origin_eori ? [{ qualifier: "EORI", number: data.origin_eori }] : undefined
        },
        receiver: {
            ...
            registrationNumbers: data.dest_eori ? [{ qualifier: "EORI", number: data.dest_eori }] : undefined
        },
        pickup: (() => {
            const fromDateStr = data.collectDateFrom || data.pickup?.collectDateFrom;
            const toDateStr = data.collectDateTo || data.pickup?.collectDateTo;

            let from = fromDateStr ? new Date(fromDateStr) : collectFrom;
            let to = toDateStr ? new Date(toDateStr) : collectTo;

            // Validation: Ensure valid dates
            if (isNaN(from.getTime())) from = collectFrom;
            if (isNaN(to.getTime())) to = collectTo;

            // Validation: Ensure range (min 2 hours)
            if (to.getTime() <= from.getTime() + (1 * 60 * 60 * 1000)) {
                to = new Date(from.getTime() + (4 * 60 * 60 * 1000));
            }

            return {
                requestPickup: data.pickup?.requestPickup !== undefined ? data.pickup.requestPickup : true,
                collectDateFrom: formatDSVDate(from),
                collectDateTo: formatDSVDate(to),
        ...
        paymentInformation: {
            ...
            dutiesAndTaxesPayment: {
                ...
                iossNumber: data.iossNumber || undefined
            }
        },
        serviceOptions: {
        ...
        proformaInvoice: (data.goodsValue && parseFloat(data.goodsValue) > 0) ? {
            proformaInvoiceHeader: {
                ...
                signature: data.invoice_signature || data.origin_contact || "Sender",
                incoterm: data.incoterms || "DAP",
                currency: data.commodity_currency || data.currencyCode || "CHF",
                typeOfExport: "PERMANENT",
                exportReason: (() => {
                    const r = data.reasonForExport || "SALE";
                    if (r === "SALE") return "SOLD";
                    if (r === "GIFT") return "GIFT";
                    if (r === "SAMPLE") return "SAMPLE";
                    if (r === "RETURN") return "REP_RET";
                    return "SOLD";
                })()
            },
        ...
        commodities: [
            {
                ...
                goodsValue: {
                    ...
                    monetaryValue: parseFloat(data.unitPrice || data.goodsValue || 0) * (parseInt(data.quantity) || 1)
                }
            }
        ]
    };
