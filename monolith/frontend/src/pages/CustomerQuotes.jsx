...
import {
    Search, Calculator, MapPin, Package, Calendar, ArrowRight,
    Loader, ChevronDown, Truck, Plus
} from 'lucide-react';
...
const CustomerQuotes = () => {
    ...
    const fetchQuoteHistory = async () => {
        try {
            const res = await dsvApi.get('/customer/quotes', authHeaders);
            if (res.data.success) {
    ...
    const handleSubmit = async (e) => {
        ...
        setQuoteResults(null);
        try {
            const response = await dsvApi.post('/customer/quotes', formData, authHeaders);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button
                    ...
                    onClick={() => { setShowForm(!showForm); setQuoteResults(null); }}
                        ...
                        <div style={{ marginTop: '2rem', borderTop: '2px solid var(--accent)', paddingTop: '1.5rem', animation: 'slideUp 0.4s ease-out' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem' }}>
                                {quoteResults.map((svc, idx) => (
                                    <div key={idx} style={{ border: '1px solid #e2e8f0', borderRadius: '12px', background: 'white', overflow: 'hidden' }}>
                ...
                <div className="card" style={{ padding: '2rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                        <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            <Calendar size={22} color="var(--accent)" /> My Quote History
                        </h3>
                                ...
                                {filteredQuotes.length === 0 && (
                                    <tr>
                                        <td colSpan="7" style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>
                                            {searchTerm ? `No quotes found matching "${searchTerm}"` : 'No quotes yet. Click "New Request" to get started.'}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
